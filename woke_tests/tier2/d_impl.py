...
class Impl(Hooks):
    def impl_deploy_panoptic_pool(
        s, v3pool: IUniswapV3Pool, owner: Account, salt: int, minTargetRarity: int = 0
    ):
        ...
        if minTargetRarity > 0:
            (bestSalt, highestRarity) = s.panopticFactory.minePoolAddress(
                v3pool.token0(),
                v3pool.token1(),
                v3pool.fee(),
                salt,
            ...
            print(bestSalt)
            print(highestRarity)
            assert highestRarity >= minTargetRarity
            salt = bestSalt
